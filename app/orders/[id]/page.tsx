        ...
        <section>
          <div className="win">
            ...
            <div className="win-body">
              ...
              <button className="form-submit" onClick={() => setShowAuth(true)}>LOGIN</button>
            </div>
          </div>
        </section>
      ...
      <div className="win">
        <div className="win-titlebar">
          ...
          <div className="win-controls">
            <a href="/dashboard" className="win-btn" style={{ textDecoration: 'none' }}>x</a>
          </div>
        </div>
        <div className="win-body">
          {!order ? (
            ...
            <div className="order-detail-layout">
              <div className="order-detail-info">
                ...
                <div className="detail-field"><span className="lbl">TYPE</span><span className="val">{order.service_type.toUpperCase()} — {order.package_name}</span></div>
                <div className="detail-field"><span className="lbl">STATUS</span>
                  <span className="val"><span className={`badge badge-${order.status === 'complete' ? 'done' : order.status === 'declined' ? 'rejected' : order.status === 'accepted' || order.status === 'in_progress' ? 'approved' : 'pending'}`}>
                    {STATUS_LABELS[order.status] || order.status.toUpperCase()}
                  </span></span>
                </div>
                {order.quoted_price && <div className="detail-field"><span className="lbl">QUOTED</span><span className="val">{order.quoted_price}</span></div>}
                {order.eta && <div className="detail-field"><span className="lbl">ETA</span><span className="val">{order.eta}</span></div>}
                {order.paypal_link && order.status === 'awaiting_payment' && (
                  <div style={{ marginTop: '16px' }}>
                    <a href={order.paypal_link} target="_blank" rel="noopener" className="form-submit" style={{ display: 'inline-block', textDecoration: 'none', fontSize: '10px' }}>
                      PAY VIA PAYPAL
                    </a>
                  </div>
                )}
                {order.decline_reason && (
                  <div className="detail-field"><span className="lbl">REASON</span><span className="val" style={{ color: 'var(--red, #e06c75)' }}>{order.decline_reason}</span></div>
                )}
                {order.progress_note && (
                  <div style={{ marginTop: '16px' }}>
                    <div className="win-label">PROGRESS</div>
                    <p style={{ fontSize: '10px', color: 'var(--text)', lineHeight: 2 }}>{order.progress_note}</p>
                  </div>
                )}
              </div>
              ...
              <div className="order-chat">
                ...
                {!canChat && order.status === 'pending' && (
                  <p style={{ fontSize: '10px', color: 'var(--dim)', marginBottom: '16px' }}>Chat opens once your order is accepted.</p>
                )}
                        ...
                        <div className="chat-quote-prompt">
                          ...
                          {order.status === 'quoted' && (
                            <div className="chat-quote-actions">
                              <button className="form-submit" style={{ fontSize: '9px', padding: '8px 14px' }} onClick={() => respondToQuote('accept_quote')}>
                                ACCEPT
                              </button>
                              <button className="btn btn-outline" style={{ fontSize: '9px', padding: '8px 14px' }} onClick={() => respondToQuote('decline_quote')}>
                                DECLINE
                              </button>
                            </div>
                          )}
                ...
                {err && <p style={{ color: 'var(--red, #e06c75)', fontSize: '9px', marginTop: '8px' }}>{err}</p>}
              </div>
            </div>
          )}
        </div>
